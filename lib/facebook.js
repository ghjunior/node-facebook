...
Facebook.prototype.makeRequest = function(host, method, path, params, callback) {
  ...
  var options = {
		...
		method: method,
		path: path,
		headers: {'host': host, 'User-Agent': 'NodeJS HTTP Client'}
  };
  
	if (method == 'GET') {
	  options.path = path + '?' + params;
	  var fb_request = https.request(options);
  } else {
    options.headers = {'host': host, 'User-Agent': 'NodeJS HTTP Client', 'Content-Length': params.length, 'Content-Type': 'application/x-www-form-urlencoded'};
    var fb_request = https.request(options);
